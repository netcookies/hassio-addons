...
for rule in $(iptables -L --line-number | grep 'Ip Forward Handler' | awk '{print $1}' | sort -r)
do
    iptables -D FORWARD ${rule} || true
done

for rule in $(iptables -t nat -L --line-number | grep 'Ip Forward Handler' | awk '{print $1}' | sort -r)
do
...
do
    ...
    iptables -A FORWARD -d ${IP} -j ACCEPT -m comment \
        --comment "Generated by Ip Forward Handler: ${IP}"
...
iptables -nvL --line-number | grep 'Ip Forward Handler' || true
iptables -nvL -t nat --line-number | grep 'Ip Forward Handler' || true
